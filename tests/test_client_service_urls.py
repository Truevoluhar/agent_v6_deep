...
def test_resolve_agent_api_url_uses_fallback_when_primary_fails(monkeypatch) -> None:
    ...
    def fake_get(url: str, params: dict[str, str] | None = None, timeout: int = 5) -> DummyResponse:
        if url.startswith("http://agent-api:8080"):
            raise requests.ConnectionError("unreachable")
        if url.startswith("http://host.docker.internal:8080") and url.endswith("/v1/threads"):
            return DummyResponse([])
        raise AssertionError(f"unexpected url {url} params={params}")

    monkeypatch.setattr(service_urls.requests, "get", fake_get)
    monkeypatch.setenv("AGENT_API_URL", "http://agent-api:8080")
    monkeypatch.setenv("AGENT_API_FALLBACK_URLS", "http://host.docker.internal:8080")

    assert service_urls.resolve_agent_api_url() == "http://host.docker.internal:8080"
...
def test_resolve_workspace_api_url_prefers_primary(monkeypatch) -> None:
    ...
    def fake_get(url: str, params: dict[str, str] | None = None, timeout: int = 5) -> DummyResponse:
        assert params == {"path": "/"}
        if url.startswith("http://workspace-api:8090") and url.endswith("/v1/files"):
            return DummyResponse([])
        raise AssertionError(f"unexpected url {url} params={params}")
